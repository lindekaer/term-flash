...
async function runOptions (newPath) {
  const file = await fs.readFile(path.join(__dirname, '..', 'config.json'), 'utf-8')
  const json = JSON.parse(file)
  // Set new path
  json.deckDir = newPath
  await fs.writeFile(path.join(__dirname, '..', 'config.json'), new Buffer(JSON.stringify(json, null, 2)))
  console.log(`
  Directory for decks has been changed to "${chalk.yellow(newPath)}". Please place your JSON decks there.
  `)
  process.exit()
}
...
export { runQuiz, runAdd, runOptions }
